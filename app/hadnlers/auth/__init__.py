    ...
    def register_role(call: CallbackQuery):
        ...
        def register_fullname(message: Message):
            ...
            def register_contact(message: Message):
                ...
                log_file.write(f"\n[INFO {datetime.datetime.now()}]: {message.contact.phone_number}, {type(message.contact.phone_number)}\n")
                log_file.close()
                ...
                new_reply_markup = customer_start_keyboard if user.role.type == 'customer' else executor_start_keyboard
